...
class Emojifier {
     ...
     *
     * @param aBackgroundBitmap The original picture
     * @param aEmojiBitmap      The chosen emoji
     * @param aFace             The detected face
     * @return The final bitmap, including the emojis over the faces
     */
    private static Bitmap addBitmapToFace(Bitmap aBackgroundBitmap, Bitmap aEmojiBitmap, Face aFace) {
        // Initialize the results bitmap to be a mutable copy of the original image
        Bitmap resultBitmap = Bitmap.createBitmap(aBackgroundBitmap.getWidth(),
                aBackgroundBitmap.getHeight(), aBackgroundBitmap.getConfig());

        // Scale the emoji so it looks better on the face
        float scaleFactor = EMOJI_SCALE_FACTOR;

        // Determine the size of the emoji to match the width of the face and preserve aspect ratio
        int newEmojiWidth = (int) (aFace.getWidth() * scaleFactor);
        int newEmojiHeight = (int) (aEmojiBitmap.getHeight() *
                newEmojiWidth / aEmojiBitmap.getWidth() * scaleFactor);


        // Scale the emoji
        aEmojiBitmap = Bitmap.createScaledBitmap(aEmojiBitmap, newEmojiWidth, newEmojiHeight, false);

        // Determine the emoji position so it best lines up with the face
        float emojiPositionX =
                (aFace.getPosition().x + aFace.getWidth() / 2) - aEmojiBitmap.getWidth() / 2;
        float emojiPositionY =
                (aFace.getPosition().y + aFace.getHeight() / 2) - aEmojiBitmap.getHeight() / 3;

        // Create the canvas and draw the bitmaps to it
        Canvas canvas = new Canvas(resultBitmap);
        canvas.drawBitmap(aBackgroundBitmap, 0, 0, null);
        canvas.drawBitmap(aEmojiBitmap, emojiPositionX, emojiPositionY, null);
