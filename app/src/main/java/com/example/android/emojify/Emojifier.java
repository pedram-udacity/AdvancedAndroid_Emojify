...
class Emojifier {
    ...
    private static final double FACE_DETECTION_THRESHOLD_SMILING = 0.4;
    private static final double FACE_DETECTION_THRESHOLD_EYE_OPEN = 0.5;
    ...
    private static void whichEmoji(Face face) {
        ...
        boolean isSmiling = face.getIsSmilingProbability() > FACE_DETECTION_THRESHOLD_SMILING;
        boolean isRightEyeOpen = face.getIsRightEyeOpenProbability() > FACE_DETECTION_THRESHOLD_EYE_OPEN;
        boolean isLeftEyeOpen = face.getIsLeftEyeOpenProbability() > FACE_DETECTION_THRESHOLD_EYE_OPEN;

        // TODO (5): Create an if/else system that selects the appropriate emoji based on the above booleans and log the result.
        Log.d(LOG_TAG, "whichEmoji: " + whichEmoji(isSmiling, isRightEyeOpen, isLeftEyeOpen));

    }

    private static Emoji whichEmoji(boolean aIsSmiling, boolean aIsRightEyeOpen, boolean aisLeftEyeOpen) {
        if (aIsSmiling) {
            if (aIsRightEyeOpen) {
                if (aisLeftEyeOpen) {
                ...
                if (aisLeftEyeOpen) {
                    ...
                    return Emoji.CLOSED_EYE_SMILING;
                }
            }
        } else {
            if (aIsRightEyeOpen) {
                if (aisLeftEyeOpen) {
                    ...
                    return Emoji.RIGHT_WINK_FROWNING;
                }
            } else {
                if (aisLeftEyeOpen) {
                    return Emoji.LEFT_WINK_FROWNING;
                } else {
                    return Emoji.CLOSED_EYE_FROWNING;
                }
            }
        }
    }
